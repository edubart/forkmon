...
/* Compile hash: 2tbrr6cMEAoxJ84mReNuuagsbj8m */
/* ------------------------------ DECLARATIONS ------------------------------ */
#ifdef __GNUC__
...
static void forkmon_kill(int code);
static void forkmon_logf_1(nlstring fmt, nlstring __arg1);
static void forkmon_logf_2(nlstring fmt, char* __arg1);
...
void forkmon_hashmap_stringboolean_rehash(forkmon_hashmap_stringboolean_ptr self, uintptr_t count) {
  ...
  for(uintptr_t i = 0U, _end = self->size; i < _end; i = i + 1) {
    nlmulret_nlusize_nlusize_nlusize _ret5 = forkmon_hashmap_stringboolean__find(self, (*forkmon_span_hashmap_node_nil___atindex(&self->nodes, i)).key);
...
nlboolean_ptr forkmon_hashmap_stringboolean_peek(forkmon_hashmap_stringboolean_ptr self, nlstring key) {
  uintptr_t node_index = ({
    nlmulret_nlusize_nlusize_nlusize _ret7 = forkmon_hashmap_stringboolean__find(self, key);
    _ret7.r1;
  });
  if(node_index != UINTPTR_MAX) {
...
void forkmon_kill(int code) {
  sys_killwait(forkmon_root_pid, 12);
  exit(code);
}
...
void forkmon_logf_2(nlstring fmt, char* __arg1) {
  if(!forkmon_quiet) {
    if(!forkmon_no_colors) {
      nelua_filestream_write_1(&nelua_io_stderr, sys_Colors_Yellow);
    }
...
bool forkmon_filter_filename(nlstring filename) {
  {
    nlmulret_function_LVr5rP9wJyrc2Fjq_nelua_sequence_string_nlint64 _ret24 = nelua_ipairs_1(forkmon_filter_patts);
...
bool forkmon_monitor_file(nlstring filename) {
  ...
  nlmulret_nlcint_nlcint _ret27 = forkmon_inotify_init_watch(nelua_string2cstring(filename), inotify_mask);
  ...
  while(true) {
    ...
    if(len > 0) {
      ...
      if(wdchange) {
        ...
        close(inotify_fd);
        nlmulret_nlcint_nlcint _ret28 = forkmon_inotify_init_watch(nelua_string2cstring(filename), inotify_mask);
...
bool forkmon_track_file_open(char* name) {
  ...
  bool _ret30 = forkmon_monitor_file(filename);
  { /* defer */
    nelua_nlstring_destroy(filename);
  }
  return _ret30;
...
void forkmon_signal_handler(int signum) {
  ...
  if(forkmon_child_pid != 0) {
    sys_killwait(forkmon_child_pid, 12);
  }
...
void forkmon_parse_env() {
  ...
  forkmon_logf_2(((nlstring){(uint8_t*)"watching files with filter \"%s\" ...", 35}), filter_env);
...
int nelua_main(int nelua_argc, char** nelua_argv) {
  ...
  {
    forkmon_assertkill((signal(12, forkmon_signal_handler) == (function_ExBujEjR8Nqr9hSv)0), ((nlstring){(uint8_t*)"failed to setup signal handler", 30}));
    forkmon_parse_env();
