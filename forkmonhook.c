...
/* Compile hash: 3yMHDtwegiyVVPJ2QLEDcdmeQjvj */
/* ------------------------------ DECLARATIONS ------------------------------ */
#ifdef __GNUC__
...
typedef nlmulret_nlboolean_nlint64_nlstring (*function_3jnQYRC7k6xoAzyW2)(nelua_sequence_string, int64_t);
typedef struct nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64 {
  function_3jnQYRC7k6xoAzyW2 r1;
  nelua_sequence_string r2;
  int64_t r3;
} nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64;
static nelua_inline nlmulret_nlboolean_nlint64_nlstring nelua_ipairs_next(nelua_sequence_string a_1, int64_t k);
static nelua_inline nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64 nelua_ipairs_1(nelua_sequence_string a);
...
typedef int (*function_3URpV1hzgYDZxoyFE)(FILE_ptr);
struct nelua_FStream {
  FILE_ptr fp;
  function_3URpV1hzgYDZxoyFE closef;
};
...
typedef nlmulret_nlboolean_nlstring_nelua_span_string (*function_V76yH6BMFarXkSNJ)(nelua_GMatchState_ptr, nlstring);
typedef struct nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring {
  function_V76yH6BMFarXkSNJ r1;
  nelua_GMatchState r2;
  nlstring r3;
} nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring;
...
static nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring nelua_nlstring_gmatchview_1(nlstring s, nlstring pattern, nlniltype init);
...
typedef int (*function_2FZKiZtmL6UUGE4jP)(FILE_ptr);
static nelua_filestream nelua_filestream__from_fp(FILE_ptr fp, function_2FZKiZtmL6UUGE4jP closef);
...
static void _nelua_assert_line28(bool cond, nlstring msg);
...
typedef void (*function_49unUCXX2dYmSB85Z)(int);
...
nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64 nelua_ipairs_1(nelua_sequence_string a) {
  return (nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64){nelua_ipairs_next, a, 0};
}
...
nlmulret_nlboolean_nlstring_nelua_span_string nelua_gmatch_next(nelua_GMatchState_ptr state, nlstring it) {
  ...
  if(e != -1) {
    ...
    _nelua_assert_line23((state->ms.numcaptures <= 8), ((nlstring){(uint8_t*)"too many captures", 17}));
...
nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring nelua_nlstring_gmatchview_1(nlstring s, nlstring pattern, nlniltype init) {
  intptr_t init_2 = 1;
  nelua_GMatchState state = (nelua_GMatchState){.ms = nelua_PatternMatcher_create(s, pattern), .init = init_2};
  return (nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring){nelua_gmatch_next, state, (nlstring){0}};
}
...
nelua_filestream nelua_filestream__from_fp(FILE_ptr fp, function_2FZKiZtmL6UUGE4jP closef) {
...
bool forkmonhook_filter_filename(nlstring filename) {
  {
    nlmulret_function_3jnQYRC7k6xoAzyW2_nelua_sequence_string_nlint64 _ret14 = nelua_ipairs_1(forkmonhook_filter_patts);
    function_3jnQYRC7k6xoAzyW2 __fornext = _ret14.r1;
    nelua_sequence_string __forstate = _ret14.r2;
    int64_t __forit = _ret14.r3;
    while(true) {
      bool __forcont;
      nlstring patt;
      nlmulret_nlboolean_nlint64_nlstring _ret15 = __fornext(__forstate, __forit);
      __forcont = _ret15.r1;
      __forit = _ret15.r2;
      patt = _ret15.r3;
      if(!__forcont) {
      ...
      {
        if(({
          nlmulret_nlisize_nlisize _ret16 = nelua_nlstring_find_1(filename, patt, NLNIL, NLNIL);
          _ret16.r1;
        }) != 0) {
          return true;
        }
...
void _nelua_assert_line24(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:57:10: runtime error: ", 1, 40, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
...
void _nelua_assert_line25(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:59:10: runtime error: ", 1, 40, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
...
nlmulret_nlcint_nlcint forkmonhook_inotify_init_watch(char* filename, int mask) {
  int fd = inotify_init();
  _nelua_assert_line24((fd >= 0), ((nlstring){(uint8_t*)"[forkmon] failed to initialize inotify", 38}));
  int wd = inotify_add_watch(fd, filename, (uint32_t)mask);
  _nelua_assert_line25((wd >= 0), ((nlstring){(uint8_t*)"[forkmon] failed to watch file", 30}));
  return (nlmulret_nlcint_nlcint){fd, wd};
}
void _nelua_assert_line26(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:71:12: runtime error: ", 1, 40, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
...
void _nelua_assert_line27(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:78:10: runtime error: ", 1, 40, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
    fwrite("\n  assert(pid >= 0, '[forkmon] fork failed')\n         ^~~~~~~~\n", 1, 63, stderr);
    nelua_abort();
  }
}
void _nelua_assert_line28(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:122:14: runtime error: ", 1, 41, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
    fwrite("\n      assert(pid >= 0, '[forkmon] fork failed')\n             ^~~~~~~~\n", 1, 71, stderr);
    nelua_abort();
  }
...
bool forkmonhook_monitor_file(nlstring filename) {
  ...
  if(forkmonhook_parent_inotify_fd >= 0) {
    ...
    _nelua_assert_line26((inotify_wd >= 0), ((nlstring){(uint8_t*)"failed to watch file", 20}));
    return false;
  }
  if(!forkmonhook_quiet) {
  ...
  int pid = fork();
  _nelua_assert_line27((pid >= 0), ((nlstring){(uint8_t*)"[forkmon] fork failed", 21}));
  if(pid == 0) {
    setpgid(0, 0);
    return false;
  }
  forkmonhook_child_pid = pid;
  nlmulret_nlcint_nlcint _ret17 = forkmonhook_inotify_init_watch(nelua_string2cstring(filename), inotify_mask);
  int inotify_fd = _ret17.r1;
  int inotify_wd = _ret17.r2;
  ...
  while(true) {
    ...
    if(len > 0) {
      ...
      if(wdchange) {
        ...
        close(inotify_fd);
        nlmulret_nlcint_nlcint _ret18 = forkmonhook_inotify_init_watch(nelua_string2cstring(filename), inotify_mask);
        inotify_fd = _ret18.r1;
        inotify_wd = _ret18.r2;
      }
      sys_killwait(pid, 12);
      pid = fork();
      _nelua_assert_line28((pid >= 0), ((nlstring){(uint8_t*)"[forkmon] fork failed", 21}));
      if(pid == 0) {
        setpgid(0, 0);
...
bool forkmonhook_track_file_open(char* name) {
  ...
  if(((nelua_nlstring___eq(filename, ((nlstring){(uint8_t*)"", 0})) || (({
    nlmulret_nlisize_nlisize _ret19 = nelua_nlstring_find_1(filename, ((nlstring){(uint8_t*)"^/tmp", 5}), NLNIL, NLNIL);
    _ret19.r1;
  ...
  bool _ret20 = forkmonhook_monitor_file(filename);
  { /* defer */
    nelua_nlstring_destroy(filename);
  }
  return _ret20;
...
void _nelua_assert_line29(bool cond, nlstring msg) {
  if(nelua_unlikely(!cond)) {
    fwrite("forkmonhook.nelua:167:10: runtime error: ", 1, 41, stderr);
    fwrite(msg.data, msg.size, 1, stderr);
...
int nelua_main(int nelua_argc, char** nelua_argv) {
  { /* require 'io' */
    ...
    nelua_io_stderr = nelua_filestream__from_fp(stderr, (function_2FZKiZtmL6UUGE4jP)NULL);
    nelua_io_stdout = nelua_filestream__from_fp(stdout, (function_2FZKiZtmL6UUGE4jP)NULL);
    nelua_io_stdin = nelua_filestream__from_fp(stdin, (function_2FZKiZtmL6UUGE4jP)NULL);
  }
  forkmonhook_root_pid = getpid();
  {
    _nelua_assert_line29((signal(12, forkmonhook_signal_handler) == (function_49unUCXX2dYmSB85Z)0), ((nlstring){(uint8_t*)"[forkmon] failed to setup signal handler", 40}));
    unsetenv("LD_PRELOAD");
    nlstring filter = nelua_cstring2string(getenv("FORKMON_FILTER"));
    if(!true) {
      forkmonhook_logf_1(((nlstring){(uint8_t*)"[forkmon] %s", 12}), ((nlstring){(uint8_t*)_strlit1, 101}));
      exit(-1);
    }
    {
      nlmulret_function_V76yH6BMFarXkSNJ_nelua_GMatchState_nlstring _ret21 = nelua_nlstring_gmatchview_1(filter, ((nlstring){(uint8_t*)"[^;]+", 5}), NLNIL);
      function_V76yH6BMFarXkSNJ __fornext = _ret21.r1;
      nelua_GMatchState __forstate = _ret21.r2;
      nlstring __forit = _ret21.r3;
      while(true) {
        bool __forcont;
        nlmulret_nlboolean_nlstring_nelua_span_string _ret22 = __fornext(&__forstate, __forit);
        __forcont = _ret22.r1;
        __forit = _ret22.r2;
        if(!__forcont) {
          break;
